...
import { updateProposalAction, generateProposalSlugAction } from '@/lib/actions/proposals'
import { formatCurrency, cn, SELECT_NONE, toSelectValue, fromSelectValue } from '@/lib/utils'
import {
...
export function ProposalEditor({ proposal, clients }: ProposalEditorProps) {
  ...
  const [totalAmount, setTotalAmount] = useState(proposal.total_amount ?? 0)
  ...
  async function handleGenerateLink() {
    const result = await generateProposalSlugAction(proposal.id)
    if (result.error) toast({ title: 'Error', description: result.error, variant: 'destructive' })
    else if (result.data) {
      setSlug(result.data)
      const url = `${window.location.origin}/share/proposal/${result.data}`
